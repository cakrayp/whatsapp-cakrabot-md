/** Made by "https://github.com/mrijoo" from indonesia.
 * 
 *  Source code in..
 *  npm package     : "https://www.npmjs.com/package/primbon-scraper"
 *  github          : "https://github.com/mrijoo/Primbon-Scraper.git"
 * 
 * ============================================================================
 */
    ...
    return new Promise((resolve, reject) => {
        axios
            ...
            .then(({
                ...
                const artinama = isi.split(/\n/)
                if (!artinama[7].trim()) throw Error('Tidak ada nama yang anda masukkan!')
                resolve({
                    nama : artinama[5].split('Nama')[1].split(',')[0].trim(),
                    meaning : artinama[5].split(':')[1].trim(),
                    description : artinama[7].trim()
                });
            })
            .catch(reject);
    });
    ...
    return new Promise((resolve, reject) => {
        ...
        axios
            ...
            .then(({
                ...
                const $ = cheerio.load(data);
                const tanggalJadian = $("#body").text().replace("Karakteristik:", "\nKarakteristik:").replace("Hubungan", "\nHubungan").split(/\n/)   //replace(/^\s*\n/gm, "").replace(/< Hitung Kembali.*$/s, "")
                if (!/^[0-9]+$/.test(tanggal.replace(/\-/g, ''))) throw Error(tanggalJadian[6])
                resolve({
                    tanggal : tanggalJadian[6].split(":")[1].trim(),
                    karakteristik : tanggalJadian[7],
                    description : tanggalJadian[8]
                })
            })
            .catch(reject);
    });
